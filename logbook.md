...
__**Fr 23-06-2017**__  
**08:30-** Got a GPU, the Tensorflow installation works, compatibility issues were surprisingly few (just `__next__()` and a path separator). The evaluation runs and MY GOD it's fast. I've added a results logger for convenience. The abstractions all work, the networks work, but `Dataset-2` runs into a `Resource exhausted: OOM when allocating tensor` error. Training works fine, so it's probably because testing doesn't apply batching. I will train on dataset-1 for now and imlement batching for training.
