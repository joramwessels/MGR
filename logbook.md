...
__**Sa 17-06-2017**__  
**09-30-12:00** Futher dropout benchmarking varies in results. The testing algorithm doesn't work when calling it explicitly. This means that the testing procedure could be distorted during training as well. Yesterday's baseline evaluation was wrong; 88% of the tracks only have 2 labels. That makes the baseline 12.5 * 2.22 = 27.75%. However, the proportions of the dataset matter in computing the random baseline. 211/394 tracks are labelled as house, which would therefore always be his best guess. The zero rule baseline accuracy of dataset-1 is 53.55%, above any of our results so far.
